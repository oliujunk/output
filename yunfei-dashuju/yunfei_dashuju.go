...
func updateToken() {
	token = xphapi.GetToken121("大数据", "123456")
}
...
func initMqttClient() {
	for _, device := range devices {
		clientOptions := mqtt.NewClientOptions().AddBroker("8.136.98.49:61883").SetUsername("yfkj").SetPassword("yfkj123456").SetClientID(fmt.Sprintf("%d", device.DeviceID))
	...
	for index, device := range devices {
		...
		if len(dataEntity.Entity) > 0 {
			...
			if datatime.After(now.Add(-time.Hour * 2)) {
				...
				clients[index].Publish(fmt.Sprintf(`/xph/qxz/pub/%d`, device.DeviceID), 1, false, message)
			}
